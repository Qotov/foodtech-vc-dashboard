...
- **Segment treemap** — squarified, area = disclosed capital or deal count.
- **Time series** — monthly capital (bars) and deal flow (line).
- **Geographic map** — country choropleth + city-hub bubbles.
- **Cross-tabs** — segment × stage and region × segment heatmaps.
- **Most active investors** — lead vs. participation split, focus segment.
- **Deal table** — sortable, searchable, with per-row confidence + verify flags.
...
This is a **curated, hand-verified core** (~68 rounds), not a market census.
It is built for trend and structure analysis. Key rules:
